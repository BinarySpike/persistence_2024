...
function read_wand(entity_id)
  ...
  for _, comp in ipairs(EntityGetAllComponents(entity_id)) do
    if ComponentGetTypeName(comp) == "AbilityComponent" then
      wand_data["shuffle"] = tonumber(ComponentObjectGetValue(comp, "gun_config", "shuffle_deck_when_empty")) == 1 and
      true or false;
...
function create_wand(wand_data)
  ...
  local entity_id = EntityLoad("mods/persistence_2024/files/wand_empty.xml", x, y);
