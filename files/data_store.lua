dofile_once("mods/persistence_2024/config.lua");
dofile_once("data/scripts/gun/gun_actions.lua");
dofile_once("data/scripts/gun/procedural/wands.lua");
dofile_once("mods/persistence_2024/files/wand_spell_helper.lua");
...
function get_player_money()
  local money = tonumber(ComponentGetValue(get_wallet(), "money")) + get_safe_money(get_selected_save_id());
  return money == nil and 0 or money;
end

function get_only_player_money()
  local money = tonumber(ComponentGetValue(get_wallet(), "money"));
  return money == nil and 0 or money;
end

function set_player_money(value)
  local player_money = tonumber(ComponentGetValue(get_wallet(), "money"));
  local safe_money = get_safe_money(get_selected_save_id())

  local money = player_money + safe_money;
  local price = money - value;

  local player_money_set = math.max(0, player_money - price)
  local safe_money_set = safe_money - price + player_money - player_money_set;
  ...
  --   money ..
  --   ", Price: " ..
  --   price .. ", Value: " .. value .. ", Player Set: " .. player_money_set .. ", Safe Set: " .. safe_money_set);

  ComponentSetValue(get_wallet(), "money", player_money_set);
  set_safe_money(get_selected_save_id(), safe_money_set)
end

function set_only_player_money(value)
...
function load(save_id)
  ...
  for i = 1, get_template_count() do
    if HasFlagPersistent(flag_prefix .. "_" .. save_id_string .. "_template_" .. tostring(i)) then
      ...
      data_store[save_id]["templates"][i]["spells_per_cast"] = hex_to_number(load_hex(save_id_string ..
        "_template_" .. tostring(i) .. "_spells_per_cast"));
      data_store[save_id]["templates"][i]["cast_delay"] = hex_to_number(load_hex(save_id_string ..
        "_template_" .. tostring(i) .. "_cast_delay"));
      data_store[save_id]["templates"][i]["recharge_time"] = hex_to_number(load_hex(save_id_string ..
        "_template_" .. tostring(i) .. "_recharge_time"));
      data_store[save_id]["templates"][i]["mana_max"] = hex_to_number(load_hex(save_id_string ..
        "_template_" .. tostring(i) .. "_mana_max"));
      data_store[save_id]["templates"][i]["mana_charge_speed"] = hex_to_number(load_hex(save_id_string ..
        "_template_" .. tostring(i) .. "_mana_charge_speed"));
      data_store[save_id]["templates"][i]["capacity"] = hex_to_number(load_hex(save_id_string ..
        "_template_" .. tostring(i) .. "_capacity"));
      data_store[save_id]["templates"][i]["spread"] = hex_to_number(load_hex(save_id_string ..
        "_template_" .. tostring(i) .. "_spread")) / 10;
...
function get_mana_charge_speed(save_id)
  ...
  return data_store[save_id]["mana_charge_speed"] == nil and mana_charge_speed_min or
      data_store[save_id]["mana_charge_speed"];
end
...
local function set_spread_min(save_id, value)
  ...
  save_hex(tostring(save_id) .. "_spread_min",
    number_to_hex(data_store[save_id]["spread_min"] == nil and nil or math.floor(data_store[save_id]["spread_min"] * 10)));
end
...
local function set_spread_max(save_id, value)
  ...
  save_hex(tostring(save_id) .. "_spread_max",
    number_to_hex(data_store[save_id]["spread_max"] == nil and nil or math.ceil(data_store[save_id]["spread_max"] * 10)));
end

-- money
function get_safe_money(save_id)
  if save_id == nil or data_store[save_id] == nil then
    return nil;
  end
...
function get_template(save_id, template_id)
  ...
  return shallowCopy(data_store[save_id]["templates"][template_id]);
end

function set_template(save_id, template_id, wand_data_orig)
  local wand_data = shallowCopy(wand_data_orig)
  if data_store[save_id] == nil or data_store[save_id]["templates"] == nil then
    return;
  end
...
function can_create_wand(save_id)
  return get_cast_delay_min(save_id) ~= nil and get_cast_delay_max(save_id) ~= nil and
      get_recharge_time_min(save_id) ~= nil and get_recharge_time_max(save_id) ~= nil and get_spread_min(save_id) ~= nil and
      get_spread_max(save_id) ~= nil;
end
...
function research_wand_price(save_id, entity_id)
  ...
  if recharge_time_min == nil or recharge_time_max == nil then
    ...
    if wand_data["recharge_time"] < recharge_time_min then
      price = price + (0.01 ^ (wand_data["recharge_time"] / 60 - 1.8) + 200) -
          (0.01 ^ (recharge_time_min / 60 - 1.8) + 200);
    end
    if wand_data["recharge_time"] > recharge_time_max then
...
function transfer_money_to_safe(save_id, amount)
  amount = math.min(get_only_player_money(), amount);

  set_safe_money(save_id, get_safe_money(save_id) + amount);
  set_only_player_money(get_only_player_money() - amount);
  return true;
end

function transfer_money_to_player(save_id, amount)
  amount = math.min(get_safe_money(save_id), amount);
