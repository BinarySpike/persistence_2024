...
dofile_once("mods/persistence_2024/files/data_store.lua");
dofile_once("mods/persistence_2024/files/helper.lua");
...
function show_money_gui()
  active_windows["money"] = { false, function(get_next_id)
    ...
    local player_money = get_only_player_money();
    ...
    active_windows["buy_wands"] = { true, function(get_next_id)
      ...
      if window_nr == 0 then
        ...
        if wand_data_selected["cast_delay"] - 60 >= cast_delay_min then
          ...
          GuiColorSetForNextWidget(gui, 1.0, 1.0, 1.0, 0.2);
          if GuiButton(gui, 0, 0, "<<<", get_next_id()) then
            wand_data_selected["cast_delay"] = cast_delay_min;
          end
        end
        if wand_data_selected["recharge_time"] - 60 >= recharge_time_min then
          ...
          GuiColorSetForNextWidget(gui, 1.0, 1.0, 1.0, 0.2);
          if GuiButton(gui, 0, 0, "<<<", get_next_id()) then
            wand_data_selected["recharge_time"] = recharge_time_min;
          end
        end
        if wand_data_selected["mana_max"] - 100 >= mana_max_min then
          ...
          GuiColorSetForNextWidget(gui, 1.0, 1.0, 1.0, 0.2);
          if GuiButton(gui, 0, 0, "<<<", get_next_id()) then
            wand_data_selected["mana_max"] = mana_max_min;
          end
        end
        if wand_data_selected["mana_charge_speed"] - 100 >= mana_charge_speed_min then
          ...
          GuiColorSetForNextWidget(gui, 1.0, 1.0, 1.0, 0.2);
          if GuiButton(gui, 0, 0, "<<<", get_next_id()) then
            wand_data_selected["mana_charge_speed"] = mana_charge_speed_min;
          end
        end
        GuiText(gui, 0, 0, " ");
        if wand_data_selected["spread"] - 10 >= spread_min then
          ...
          GuiColorSetForNextWidget(gui, 1.0, 1.0, 1.0, 0.2);
          if GuiButton(gui, 0, 0, "<<<", get_next_id()) then
            wand_data_selected["spread"] = spread_min;
          end
        end
        GuiText(gui, 0, 0, "      ");
        GuiLayoutEnd(gui);
        ...
        if wand_data_selected["capacity"] - 10 >= capacity_min then
          ...
          GuiColorSetForNextWidget(gui, 1.0, 1.0, 1.0, 0.2);
          if GuiButton(gui, 0, 0, "<<", get_next_id()) then
            wand_data_selected["capacity"] = capacity_min;
          end
        end
        ...
        if wand_data_selected["cast_delay"] + 6 <= cast_delay_max then
          ...
          GuiColorSetForNextWidget(gui, 1.0, 1.0, 1.0, 0.2);
          if GuiButton(gui, 0, 0, ">>", get_next_id()) then
            wand_data_selected["cast_delay"] = cast_delay_max;
          end
        end
        if wand_data_selected["recharge_time"] + 6 <= recharge_time_max then
          ...
          GuiColorSetForNextWidget(gui, 1.0, 1.0, 1.0, 0.2);
          if GuiButton(gui, 0, 0, ">>", get_next_id()) then
            wand_data_selected["recharge_time"] = recharge_time_max;
          end
        end
        if wand_data_selected["mana_max"] + 10 <= mana_max then
          ...
          GuiColorSetForNextWidget(gui, 1.0, 1.0, 1.0, 0.2);
          if GuiButton(gui, 0, 0, ">>", get_next_id()) then
            wand_data_selected["mana_max"] = mana_max;
          end
        end
        if wand_data_selected["mana_charge_speed"] + 10 <= mana_charge_speed then
          ...
          GuiColorSetForNextWidget(gui, 1.0, 1.0, 1.0, 0.2);
          if GuiButton(gui, 0, 0, ">>", get_next_id()) then
            wand_data_selected["mana_charge_speed"] = mana_charge_speed;
          end
        end
        if wand_data_selected["capacity"] + 10 <= capacity then
          ...
          GuiColorSetForNextWidget(gui, 1.0, 1.0, 1.0, 0.2);
          if GuiButton(gui, 0, 0, ">>", get_next_id()) then
            wand_data_selected["capacity"] = capacity;
          end
        end
        if wand_data_selected["spread"] + 1 <= spread_max then
          ...
          GuiColorSetForNextWidget(gui, 1.0, 1.0, 1.0, 0.2);
          if GuiButton(gui, 0, 0, ">>", get_next_id()) then
            wand_data_selected["spread"] = spread_max;
          end
        end
        GuiText(gui, 0, 0, "    ");
        GuiLayoutEnd(gui);
  ...
  if gui ~= nil then
    if active_windows ~= nil then
      ...
      if is_dark_background then
        local cx, cy = GameGetCameraPos();
        GameCreateSpriteForXFrames("mods/persistence_2024/files/gui_darken.png", cx, cy);
      end
